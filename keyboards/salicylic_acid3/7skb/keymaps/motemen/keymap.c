...
enum my_keycodes {
    MY_RSHIFT = SAFE_RANGE,
};

// shift + ^ -> &
const key_override_t rshift_key_override = ko_make_basic(MOD_MASK_SHIFT, KC_CIRC, KC_AMPR);
...
// https://blog.bouzuya.net/2017/09/08/
// clang-format off
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  //|--------+--------+--------+--------+--------+--------|   |--------+--------+--------+--------+--------+--------+--------|
      KC_LSFT,    KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,        KC_N,    KC_M, KC_COMM,  KC_DOT, KC_SLSH, KC_CIRC, MO(_FN),
  //|--------+--------+--------+--------+--------+--------|   |--------+--------+--------+--------+--------+--------+--------|
               KC_LALT, CMD_T(KC_LNG2), KC_SPC, KC_SPC,          KC_F12,  KC_SPC,          KC_RGUI, LALT(LCMD(KC_X))
          //`---------------------------------------------|   |--------------------------------------------'
  ),
    ...
    if (keycode == MY_RSHIFT) {
        ...
        if (lshifted) {
            if (record->event.pressed) {
                ...
                tap_code((uint8_t)KC_AMPR);
            }
            register_code(KC_LSFT);
        } else {
            if (record->event.pressed) {
                // register_code(KC_LSFT);
                tap_code((uint8_t)KC_CIRC);
            }
        }
        return false;
    }
